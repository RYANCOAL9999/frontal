...
try:
    ...
    console.log("[bold green]Successfully imported Cythonized image_processor from exlib.pyc.[/bold green]")
except ImportError:
    from exlib.py.image_processor import process_image_data_intensive
    console.log("[bold yellow]Cythonized image_processor not found. Using pure Python version from exlib.py.[/bold yellow]")
...
async def process_jobs_worker(job_queue: asyncio.Queue, db_session_factory, db_crop_job_model, loadtest_mode_enabled: bool):
    ...
    while True:
        try:
            ...
            try:
                db_job = db.query(db_crop_job_model).filter(db_crop_job_model.job_id == job_id).first()

                if not db_job:
                    console.log(f"[warning]Job {job_id} not found in DB, skipping processing.[/warning]")
                    job_failed_counter.inc()
                    continue

                if db_job.status == "completed":
                    console.log(f"[info]Job {job_id} already completed, skipping reprocessing.[/info]")
                    job_completed_counter.inc()
                    continue

                if not loadtest_mode_enabled:
                    console.log(f"[info]Simulating processing for job {job_id} (20-second delay)...[/info]")
                    await asyncio.sleep(20)
                else:
                    console.log(f"[bold magenta]Load testing mode: Skipping artificial delay for job {job_id}.[/bold magenta]")
                ...
                console.log(f"[success]Job {job_id} processing completed and results stored.[/success]")
                job_completed_counter.inc()
            ...
            console.log(f"[error]Unexpected error in job processing worker: {e}[/error]")
...
async def shutdown_worker(app_instance):
    ...
    if hasattr(app_instance.state, "job_processing_task"):
        ...
        try:
            ...
            console.log("[info]Background job processing worker stopped gracefully.[/info]")
        except Exception as e:
